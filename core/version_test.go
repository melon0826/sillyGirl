...
func TestCompiledAndCurrentAppVersion(t *testing.T) {
	...
	compiled_at = "v0.1.5"
	if got := compiledAppVersion(); got != "v0.1.5" {
		t.Fatalf("compiledAppVersion() = %q, want %q", got, "v0.1.5")
	}
	if got := currentAppVersion(); got != "0.1.5" {
		t.Fatalf("currentAppVersion() = %q, want %q", got, "0.1.5")
	}
